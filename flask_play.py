...
def get_spotify_values(rank):
    ...
    def myFunc(e): #making this graph to be numerical order (largest to smallest)
        return e[2]
...
def detailedinfo():
    ...
    if (plot):
        ...
        bars_data = go.Bar(
            x=x_vals,
            y=y_vals,
            marker = {'color':y_vals, 'colorbar':dict(),'showscale':True,'colorscale':'Viridis'}) #change default color and created a color scale
